...
v0.2.3 只压缩间距，v0.2.4 起改为结构上禁止换行（纯插件侧，不修改 dsh 文件）。
v0.2.5 起单行适配与徽标解耦，**对所有模型生效**：

- 模型选择器存在时，从模型按钮向上找到 `<hash>_row` 底行（爬 8 层，兼容 slot 包装层），
  打标记类 `dsh-gq-row`；百分比徽标本身仍然仅 GLM 模型显示；
- `@media (max-width:600px)` 内对该标记行：`flex-wrap:nowrap`；右侧簇由 `flex:none`
  改为 `flex:0 1 auto;min-width:0`（可收缩）；权限选择按钮允许省略号；行内 gap 压到 8px；
- 空间不足时的收缩顺序：模型名先省略 → 权限文案省略 → 发送键固定不动，
  两行在几何上不可能出现；中英文界面同一套规则，与语言无关；
- 桌面端、宽屏完全不受影响；选择器全部 scoped 在标记行内。
