...
适配方式（纯插件侧，不修改 dsh 文件）：

- 徽标可见（选中 GLM 模型）时，从模型按钮向上找到 `<hash>_row` 底行，打标记类 `dsh-gq-row`；
  切到非 GLM 模型时移除；
- 注入的 CSS 用 `@media (max-width:600px)` 仅在窄屏压缩该行内间距：行 gap 与右侧簇
  12→8px、工具组 16→8px、模式组 12→8px，合计回收约 24px；
- 桌面端、宽屏、非 GLM 状态完全不受影响；选择器用 `[class$='_tools']` 等后缀匹配，
  且全部 scoped 在标记行内，不影响页面其他区域。
